...
const SellerOrderListPage = () => {
  ...
  const handleStatusFilterChange = async (e) => {
    ...
    try {
      ...
      let data;
      if (filterMember) {
        data = await fetchUserOrdersByStatus(
          filterMember,
  ...
  const handleMemberFilterChange = async (e) => {
    ...
    try {
      ...
      if (memberNumber) {
        data =
          filterStatus === 'ALL'
            ? await fetchUserOrders(memberNumber)
            : await fetchUserOrdersByStatus(memberNumber, filterStatus);
      } else {
        data =
          filterStatus === 'ALL'
            ? await fetchAllOrders()
            : await fetchOrdersByStatus(filterStatus);
      }
      setFilteredOrders(data);
    } catch (err) {
      console.error('Error filtering orders by member:', err);
      setError(err.message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={styles.orderListContainer}>
      ...
      <table className={styles.orderTable}>
        <thead>
          <tr>
            <th onClick={() => sortOrders('ono')}>
              주문번호 {getSortArrow('ono')}
            </th>
            <th onClick={() => sortOrders('orderDate')}>
              주문일시 {getSortArrow('orderDate')}
            </th>
            <th onClick={() => sortOrders('o_totalPrice')}>
              총금액 {getSortArrow('o_totalPrice')}
            </th>
            <th onClick={() => sortOrders('status')}>
              결제상태 {getSortArrow('status')}
            </th>
            <th onClick={() => sortOrders('mno')}>
              회원번호 {getSortArrow('mno')}
            </th>
            <th>배송지</th>
            <th>수령인</th>
            <th>수령인 전화번호</th>
          ...
          {filteredOrders.length > 0 ? (
            filteredOrders.map((order) => (
              <tr key={order.ono}>
                ...
                <td>
                  {order.refundDTO?.rf_requestDate
                    ? new Date(order.refundDTO.rf_requestDate).toLocaleString()
                    : '-'}
                </td>
                <td>
                  {order.refundDTO?.rf_completeDate
                    ? new Date(order.refundDTO.rf_completeDate).toLocaleString()
                    : '-'}
                </td>
                <td>
                  <button
                    ...
                    disabled={order.status === 'REFUND_COMPLETE'}
                    className={
                      order.status === 'REFUND_COMPLETE'
            ...
            <tr>
              <td colSpan="11" className={styles.noOrders}>
                주문 내역이 없습니다.
              </td>
            </tr>
          )}
